<script lang="ts">
	...
	// Keep the initial src to identify uploaded files
	const initialSrc: string | undefined = (node?.attrs as any)?.src;

	async function deleteFromServer(url?: string) {
		try {
			if (!url || typeof url !== 'string') return;
			if (!url.startsWith('/uploads/')) return; // only our uploads
	...
	onDestroy(() => {
		...
		// Fallback: if node is removed from editor, attempt to delete server file
