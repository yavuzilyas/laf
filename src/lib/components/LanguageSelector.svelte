<script lang="ts">
	...
	const triggerContent = $derived(localeNames[currentLocale] || 'Diller');
</script>
...
>
	<Select.Trigger
		size="sm"
		class="text-secondary-foreground  !bg-background/44 w-full md:w-32 !text-xs font-bold"
	>
		{triggerContent}
	</Select.Trigger>
	<Select.Content class="!bg-background/44 -bottom-2 !backdrop-blur-sm !text-xs font-bold">
		<Select.Group>
			...
			{#each t.availableLocales as locale (locale)}
