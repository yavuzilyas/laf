...
>
  <div class="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
    <h2 class="text-lg md:text-4xl mb-4 text-neutral-500 max-w-4xl">
      Kademe kademe seyahatimiz:
    </h2>
  </div>
  ...
  <div class="relative max-w-7xl mx-auto pb-20 overflow-hidden">
    {#each timelineData as item, index}
      <div class="flex justify-start pt-10 md:pt-40 md:gap-10">
        ...
        >
          <div
            class="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-black flex items-center justify-center"
          >
            <div
              class="h-4 w-4 rounded-full bg-yellow-200 border border-yellow-700 p-2"
            ></div>
          </div>
          <h3
    ...
    <div
      ...
      class="absolute md:left-8 left-8 top-0 overflow-hidden w-[2px] bg-[linear-gradient(to_bottom,var(--tw-gradient-stops))] from-transparent from-[0%] via-secondary dark:via-secondary to-transparent to-[99%] timeline-line"
    >
      <div
        style="height: {$heightTransform}px; opacity: {$opacityTransform};"
        class="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-orange-500 via-yellow-500 to-transparent from-[0%] via-[10%] rounded-full"
      ></div>
    </div>
  </div>
</div>
