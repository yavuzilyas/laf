...
const sanitizeRelationEntries = <T extends Record<string, any>>(
  ...
  return entries.map((entry) => ({
    ...
    ...(dateKey
      ? {
        [dateKey]: (entry as any)[dateKey] instanceof Date
          ? ((entry as any)[dateKey] as Date).toISOString()
          : (entry as any)[dateKey]
      }
      : {})
  }));
};
...
export const load = async ({ params, locals }: Parameters<PageServerLoad>[0]) => {
  ...
  // Build author object using cached data from article first, fallback to user data
  let authorInfo: { id: string; nickname?: string; name?: string; surname?: string; username?: string } | null = null;
  if (article.author_id) {
    authorInfo = {
      id: String(article.author_id),
      nickname: article.author_nickname || authorDoc?.nickname,
      name: article.author_full_name || authorDoc?.name,
      surname: article.author_surname || authorDoc?.surname,
      username: article.author_username || authorDoc?.username
    };
  }
  ...
  if (authorDoc) {
    ...
    profileUser = {
      ...
      bannerColor: authorDoc.banner_color || '#0f172a',
      bannerImage: authorDoc.banner_image || '',
  ...
  const totalLikes = userArticles.reduce((sum: number, a: any) => sum + (a.likes_count || 0), 0);
  const totalComments = userArticles.reduce((sum: number, a: any) => sum + (a.comments_count || 0), 0);
