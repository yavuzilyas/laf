...
export const updateUser = async (id: string, updates: any) => {
    ...
    const values = fields.map(key => {
        if (key === 'preferences') return JSON.stringify(updates[key]);
...
export const getArticles = async (filters: any = {}) => {
    let sql = `
        SELECT a.*, 
               ...
               COALESCE(a.author_nickname, u.nickname) as author_nickname,
...
export const createArticle = async (articleData: any) => {
    ...
    const sql = `
        INSERT INTO articles (
            id, author_id, author_username, author_nickname, translations, category, subcategory, tags, thumbnail,
            status, default_language, views, likes_count, comments_count,
            is_hidden, pending_review, metadata, created_at, updated_at, published_at
        )
    ...
    const result = await query(sql, [
        ...
        articleData.author_username || articleData.authorUsername,
        articleData.author_nickname || articleData.authorNickname,
...
export const markNotificationRead = async (userId: string, notificationId: string) => {
    const sql = 'UPDATE notifications SET is_read = true, updated_at = NOW() WHERE id = $1 AND user_id = $2';
...
export const markMultipleNotificationsRead = async (userId: string, notificationIds: string[]) => {
    ...
    const sql = `UPDATE notifications SET is_read = true, updated_at = NOW() WHERE id IN (${placeholders}) AND user_id = $1`;
    const result = await query(sql, [userId, ...notificationIds]);
    return result.rowCount;
};

export const markAllNotificationsRead = async (userId: string) => {
    const sql = 'UPDATE notifications SET is_read = true, updated_at = NOW() WHERE user_id = $1 AND is_read = false';
