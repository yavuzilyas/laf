<script lang="ts">
    ...
    function applyFiltersAndSearch() {
        ...
        // Apply nickname filter (author username/nickname/name)
        if (activeFilters.nickname && activeFilters.nickname.trim()) {
            const nicknameLower = activeFilters.nickname.toLowerCase().trim();
            result = result.filter(article => 
                article.author && 
                ((article.author.username && article.author.username.toLowerCase().includes(nicknameLower)) ||
                 (article.author.nickname && article.author.nickname.toLowerCase().includes(nicknameLower)) ||
                 (article.author.name && article.author.name.toLowerCase().includes(nicknameLower)))
            );
        }
