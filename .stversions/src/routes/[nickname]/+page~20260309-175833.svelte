<script lang="ts">
    ...
    const profileusernameSlug = $derived(() => {
        const value = profileUser?.username || profileUser?.nickname || profileUser?.id || 'user';
        return value
            .toString()
            .trim()
            .toLowerCase()
    ...
    const getProfileDisplayUsername = (user: any) => {
        if (!user) return '';
    ...
    // Load follow/block status on component mount (client-side only)
    onMount(() => {
...
<svelte:head>
    <title>@{getProfileDisplayUsername(profileUser)} - LAF</title>
    <meta name="description" content={profileUser?.bio || `${getProfileDisplayUsername(profileUser)}'s profile on LAF`} />
</svelte:head>
    ...
    <div class="container mx-auto px-3 py-4 sm:px-4 sm:py-8 max-w-6xl">
        ...
        <Card>
            <CardHeader class="space-y-2">
                ...
                <CardDescription>
                    {isOwnProfile ? t('profile.myArticlesDescription') : `@${getProfileDisplayUsername(profileUser)}'s articles`}
                </CardDescription>
            </CardHeader>
