        ...
        <div class="relative">
          <Avatar class="h-21 w-21 sm:h-32 sm:w-32 rounded-full border-background shadow-lg transition-all duration-300 ease-in-out {isEditing ? 'scale-110 shadow-xl' : 'scale-100 hover:scale-105'}">
            ...
            <AvatarFallback class="text-3xl">
              {(getDisplayUsername(profileUser) || 'U')[0].toUpperCase()}
            </AvatarFallback>
          </Avatar>
      ...
      <div class="flex-1 -mt-10 sm:-mt-3 space-y-4 pt-8 md:pt-4 transition-all duration-1200 ease-in-out flex flex-col">
        <div>
          <div class="flex items-center gap-1">
            ...
            {#if showProfileLink && getProfileUrl(profileUser)}
              <a 
                href={`/${getProfileUrl(profileUser)}`}
          ...
          <p class="text-muted-foreground">@{getDisplayUsername(profileUser)} </p>
        </div>
          ...
          <div class="space-y-4 opacity-0 animate-slideDown">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <Input id="name" bind:value={profileData.name} placeholder="{t('Name')}" class="transition-all duration-300 ease-in-out " />
              </div>
              <div>
                <Label for="surname">{t('Surname')}</Label>
                <Input id="surname" bind:value={profileData.surname} placeholder="{t('Surname')}" class="transition-all duration-300 ease-in-out " />
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <Input id="location" bind:value={profileData.location} placeholder={t('profile.location')} class="transition-all duration-300 ease-in-out " />
              </div>
              <div>
                <Label for="website">{t('profile.website')}</Label>
                <Input id="website" bind:value={profileData.website} type="url" placeholder="https://" class="transition-all duration-300 ease-in-out " />
              </div>
            </div>
<!-- 
            <div>
                ...
                <div class="relative">
                    <Avatar class="h-40 w-40">
                        {#if profileData.avatar}
                            <AvatarImage 
                                src={profileData.avatar} 
                                alt={profileData?.username || profileUser?.username || profileUser?.nickname || 'Avatar'} 
                                class="object-cover" 
                            />
                        {/if}
                        <AvatarFallback>
          ...
          {#each followersList ?? [] as follower (follower.id)}
            <li class="flex items-center justify-between gap-3 rounded-lg border p-3">
              <div class="flex max-w-[calc(100%-4rem)] items-center gap-3 flex-1">
                <a href={`/${getProfileUrl(follower)}`}>
                <Avatar class="h-10 w-10">
                  {#if follower.avatar}
                    <AvatarImage src={follower.avatar} alt={follower?.username || follower?.nickname || 'Avatar'} class="object-cover" />
                  {/if}
                  <AvatarFallback>
                    {(getDisplayUsername(follower) || 'U')[0].toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                </a>
                <div class="flex flex-col text-sm flex-1">
                  <a href={`/${getProfileUrl(follower)}`} class="font-semibold text-foreground hover:text-primary transition-colors">{getDisplayName(follower)}</a>
                  <a href={`/${getProfileUrl(follower)}`} class="text-muted-foreground hover:text-primary transition-colors">@{getDisplayUsername(follower)}</a>
                </div>
              </div>
          ...
          {#each followingList ?? [] as following (following.id)}
            <li class="flex items-center justify-between gap-3 rounded-lg border p-3">
              <div class="flex items-center gap-3 flex-1">
                <a href={`/${getProfileUrl(following)}`}>
                  <Avatar class="h-10 w-10">
                    {#if following.avatar}
                      <AvatarImage src={following.avatar} alt={following?.username || following?.nickname || 'Avatar'} class="object-cover" />
                    {/if}
                    <AvatarFallback>
                      {(getDisplayUsername(following) || 'U')[0].toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                </a>
                <div class="flex flex-col text-sm flex-1">
                  <a href={`/${getProfileUrl(following)}`} class="font-semibold text-foreground hover:text-primary transition-colors">{getDisplayName(following)}</a>
                  <a href={`/${getProfileUrl(following)}`} class="text-muted-foreground hover:text-primary transition-colors">@{getDisplayUsername(following)}</a>
                </div>
              </div>
