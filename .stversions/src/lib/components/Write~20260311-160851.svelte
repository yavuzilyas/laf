...
import { onMount, onDestroy } from 'svelte';
  ...
  onMount(async () => {
    ...
    if (user && user.id) {
      ...
      // Also store author username and nickname for saving
      articleEditor.updateMetadata('authorUsername', user.username);
      articleEditor.updateMetadata('authorNickname', user.nickname);
    }
  });
